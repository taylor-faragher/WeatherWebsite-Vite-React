import {FullImage} from 'src/types/types';

const getPicAltText = (pic: string): string => {
...
export const formPictureObject = async (icon: string): Promise<FullImage> => {
    return {image: `/assets/${icon}.svg`, imageAltText: await getPicAltText(icon)};
};
